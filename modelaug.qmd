...
On verra dans la suite que la condition ([-@eq-app2]) est l'invasion des parasites.
...
Dans ce cas ([-@eq-eq-modelau]) devient : 
...
Si $\beta = \infty$ on va réduire le système ([-@eq-modelaueg]) en équation de Lotka-Volterra avec croissance logistique. Ainsi la présence d'un équilibre des prédateurs impliquera la condition d'invasion supposée.

La démarche à partir du système ([-@eq-modelaueg]) est la suivante :
...
On va simuler numériquement la somme des équilibres des proies du système ([-@eq-modelau]) par la même méthode que précédemment en faisant aussi aparaître la courbe $n_xx^*_0+n_yy^*_0$ et $m$ ainsi que la somme des équilibres des proies du système ([-@eq-modelau]) sans prédateurs pour pouvoir voir quand on a disparition des prédateurs et donc $x^*_0 + y^*_0 = x^*+y^*$.
...
*Notre analyse s'appuie sur un modèle émis par [Freedman et Waltman](http://dx.doi.org/10.1137/0132052) dans leur article de 1977.*  