...
- Si $\frac{\gamma_y}{\gamma_x}<\frac{K_y}{K_x}$ (condition d'inflation et de déflation du modèle précédent) alors nous avons soit un système inflationiste si $\frac{m}{n} < K_x+K_y$ soit un système inflationiste et déflationniste mais moins inflationiste que sans les prédateurs si $\frac{m}{n} < (x^*_0+y^*_0)_{max}$ (autrement dit la courbe grise est au dessus de la droite orange).  
  
- Si $\frac{r_xK_y}{r_yK_x} < \frac{\gamma_y}{\gamma_x} < \frac{K_y}{K_x}$ (conditions d'inflation du modèle précédent) alors si nous avons $x^*(\infty)+y^*(\infty) > \frac{m}{n} > K_x+K_y$ alors le système est inflationiste mais moins que son [pendant sans prédateurs](modeleavecmigrationass.qmd).
...
Nous allons montrer deux cas, l'un avec $n_xx^*_0 + n_yy^*_0 = m$ décroissant et l'autre avec $n_xx^*_0 + n_yy^*_0 = m$ croissant :
```{python}
#| code-fold: true
#Définition des paramètres
#3:
...
#Boucle pour plot en fonction de beta:
for i in np.arange(0, 4, 0.01):
    s3p = odeint(a, e0p, t, args = (param31p, i)) #Nous calculons pour chaque beta l'équilibre
    s3sp = odeint(aug2, e0sp, t, args = (param31sp, i))
    
    ax.plot(i, s3p[-1][0] + s3p[-1][1], marker = '.', markersize = 2, color = 'C0') #Nous représentons un point qui correspond à la valeur de la somme des éq. en fonction de beta
    ...
    s3p = odeint(a, e0p, t, args = (param32p, i)) #Nous calculons pour chaque beta l'équilibre
    s3sp = odeint(aug2, e0sp, t, args = (param32sp, i))
    
    ax1.plot(i, s3p[-1][0] + s3p[-1][1], marker = '.', markersize = 2, color = 'C0') #Nous représentons un point qui correspond à la valeur de la somme des éq. en fonction de beta
...
Nous observons que si $m$ est toujours plus grand que $n_xx_0^*+n_yy_0^*$ alors les prédateurs disparaissent et nous avons que la somme des équilibres est égale à $x_0^*+y_0^*$ (la courbe bleu est tout le temps confondue avec la courbe grise).  

Ensuite, quand $m$ est toujours plus petit que $n_xx_0^*+n_yy_0^*$ alors les prédateurs sont toujours en vie et nous avons que la somme des équilibres est égale à $x^*+y^*$.  

Enfin, quand $m$ croise $n_xx_0^*+n_yy_0^*$ nous observons que la somme des équilibres est à la fois $x^*+y^*$ et $x_0^*+y_0^*$. Si $n_xx_0^*+n_yy_0^*$ est croissant alors nous avons d'abord que la somme des équilibres est égale à $x_0^*+y_0^*$ puis $x^*+y^*$ et inversement si $n_xx_0^*+n_yy_0^*$ est décroissante.  

### Equilibre de la culture en fonction de $\gamma_x$ et $\gamma_y$

Nous allons maintenant tracer l'équilibre dans la culture en fonction du taux de migration et de fuite.  
...
Nous observons des résultats similaires au système simplifié. L'analyse qui en résulte est la même.  
