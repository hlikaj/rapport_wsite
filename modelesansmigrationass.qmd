...
Ce qui montre que en général on a pas $x^*+y^* = K_x + K_x$. De plus par ([-@eq-inf]), si $K_y<K_x$ (culture remontante) et si $r_xK_y<r_yK_x$ alors $x^*+y^* < K_x+K_y$ ce qui nous intéresse particulièrement pour la lutte biologique (et inversement pour les cultures non remontantes).  
...
Par définition de ([-@eq-pdpsmig]) on a quand $\beta \to \infty$, $y^* = x^*$. La pente quand $\beta = 0$ est $\frac{K_y}{K_x}$. 
